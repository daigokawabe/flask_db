...
def viewall():
    ...
    # order_by() -- how do you want the data to be arranged, arrange in alphabetical order using the NAME
    user = User.query.order_by(User.name).all()
    # user = User.query.order_by(User.address).all()
...
def delete(id):
    ...
    try:
        db.session.delete(user_to_delete)   # session.delete() <--- is equivalent to DELETE FROM in mysql
        db.session.commit()     # commit() means execute/run/go
